...
BOOST_AUTO_TEST_CASE(GoBlockIteratorTest) {
  ...
  setup.AddWhite(Pt(GO_MAX_SIZE, GO_MAX_SIZE));
  GoBoard bd(GO_MAX_SIZE, setup);
  ...
  BOOST_CHECK(it);
  ++it;
  BOOST_CHECK_EQUAL(*it, Pt(GO_MAX_SIZE, GO_MAX_SIZE));
  BOOST_CHECK(it);
  ++it;
  BOOST_CHECK(!it);
}
...
BOOST_AUTO_TEST_CASE(GoBoardUtilTest_AddWall) {
  if (GO_MAX_SIZE >= 19) {
    GoBoard bd(19);
    AddWall(bd, SG_BLACK, Pt(3, 3), 10, GO_NORTH_SOUTH);
    BOOST_CHECK_EQUAL(bd.TotalNumStones(SG_BLACK), 10);
    BOOST_CHECK_EQUAL(bd.TotalNumStones(SG_WHITE), 0);
    AddWall(bd, SG_WHITE, Pt(4, 3), 5, GO_NORTH_SOUTH + GO_WEST_EAST);
    BOOST_CHECK_EQUAL(bd.TotalNumStones(SG_BLACK), 10);
    BOOST_CHECK_EQUAL(bd.TotalNumStones(SG_WHITE), 5);
    AddWall(bd, SG_WHITE, Pt(5, 3), 4, GO_NORTH_SOUTH + 2 * GO_WEST_EAST);
    BOOST_CHECK_EQUAL(bd.TotalNumStones(SG_BLACK), 10);
...
BOOST_AUTO_TEST_CASE(GoBoardUtilTest_AdjacentBlocks_2) {
  ...
  AdjacentBlocks(bd, Pt(1, 1), 4, &blocks);
  BOOST_CHECK_EQUAL(blocks.Length(), 2);
  BOOST_CHECK(blocks.Contains(Pt(3, 1)));

  AdjacentBlocks(bd, Pt(3, 2), 2, &blocks);
  BOOST_CHECK_EQUAL(blocks.Length(), 1);
  BOOST_CHECK(blocks.Contains(Pt(1, 2)));
}
...
BOOST_AUTO_TEST_CASE(GoBoardUtilTest_AllLegalMoves) {
  ...
  BOOST_CHECK_EQUAL(moves4.Length(), 22);
  bd.Play(GO_PASS);
  GoPointList moves5(AllLegalMoves(bd));
  BOOST_CHECK_EQUAL(moves5.Length(), 23);
}
...
BOOST_AUTO_TEST_CASE(GoBoardUtilTest_GainsLiberties) {
  ...
  const GoPoint anchor1 = bd.Anchor(Pt(3, 5));
...
BOOST_AUTO_TEST_CASE(GoBoardUtilTest_GetDirtyRegion) {
  ...
  BOOST_CHECK_EQUAL(s1, GoRect(3, 3, 3, 3));
  BOOST_CHECK_EQUAL(l1, GoRect(3, 3, 3, 3));
  BOOST_CHECK_EQUAL(s2, GoRect(3, 3, 3, 3));
  BOOST_CHECK_EQUAL(l2, GoRect(3, 3, 3, 3));
  ...
  BOOST_CHECK_EQUAL(s1, GoRect(3, 3, 4, 4));
  BOOST_CHECK_EQUAL(l1, GoRect(3, 3, 3, 4));
  BOOST_CHECK_EQUAL(s2, GoRect(3, 3, 4, 4));
  BOOST_CHECK_EQUAL(l2, GoRect(3, 3, 3, 4));
  ...
  BOOST_CHECK_EQUAL(s1, GoRect(4, 4, 4, 4));
  BOOST_CHECK_EQUAL(l1, GoRect(3, 4, 4, 4));
  BOOST_CHECK_EQUAL(s2, GoRect(4, 4, 4, 4));
  BOOST_CHECK_EQUAL(l2, GoRect(3, 4, 4, 4));
  ...
  BOOST_CHECK_EQUAL(s1, GoRect(4, 4, 5, 5));
  BOOST_CHECK_EQUAL(l1, GoRect(4, 4, 4, 5));
  BOOST_CHECK_EQUAL(s2, GoRect(4, 4, 5, 5));
  BOOST_CHECK_EQUAL(l2, GoRect(4, 4, 4, 5));
  ...
  BOOST_CHECK_EQUAL(s1, GoRect(2, 2, 4, 4));
  BOOST_CHECK_EQUAL(l1, GoRect(2, 3, 4, 4));
  BOOST_CHECK_EQUAL(s2, GoRect(2, 2, 4, 4));
  BOOST_CHECK_EQUAL(l2, GoRect(2, 3, 4, 4));

  s1 = GetDirtyRegion(bd, GO_PASS, SG_WHITE, false, true);
  l1 = GetDirtyRegion(bd, GO_PASS, SG_WHITE, true, true);
  bd.Play(GO_PASS, SG_WHITE);
  s2 = GetDirtyRegion(bd, GO_PASS, SG_WHITE, false, false);
  l2 = GetDirtyRegion(bd, GO_PASS, SG_WHITE, true, false);
  BOOST_CHECK_EQUAL(s1, GoRect());
  BOOST_CHECK_EQUAL(l1, GoRect());
  BOOST_CHECK_EQUAL(s2, GoRect());
  BOOST_CHECK_EQUAL(l2, GoRect());
  ...
  BOOST_CHECK_EQUAL(s1, GoRect(3, 3, 4, 5));
  BOOST_CHECK_EQUAL(l1, GoRect(2, 4, 3, 5));
  BOOST_CHECK_EQUAL(s2, GoRect(3, 3, 4, 5));
  BOOST_CHECK_EQUAL(l2, GoRect(2, 4, 3, 5));
}

void TestNb(const GoBoard& bd, GoPoint p, int nuNb) {
  int nu = 0;
...
BOOST_AUTO_TEST_CASE(GoBoardUtilTest_NeighborsOfColor) {
  ...
  setup.AddWhite(Pt(1, 2));
  GoBoard bd(9, setup);
  GoArrayList<GoPoint, 4> neighbors;
...
BOOST_AUTO_TEST_CASE(GoBoardUtilTest_OtherLiberty) {
  ...
  GoBoard bd(boardSize, setup);
  {
    const GoPoint anchor = bd.Anchor(Pt(1, 1));
    const GoPoint lib1 = Pt(1, 2);
    const GoPoint lib2 = Pt(2, 1);
    BOOST_CHECK_EQUAL(OtherLiberty(bd, anchor, lib1), lib2);
    BOOST_CHECK_EQUAL(OtherLiberty(bd, anchor, lib2), lib1);
  }
  {
    const GoPoint anchor = bd.Anchor(Pt(5, 1));
    const GoPoint lib1 = Pt(4, 1);
    const GoPoint lib2 = Pt(5, 2);
    BOOST_CHECK_EQUAL(OtherLiberty(bd, anchor, lib1), lib2);
    BOOST_CHECK_EQUAL(OtherLiberty(bd, anchor, lib2), lib1);
  }
  {
    const GoPoint anchor = bd.Anchor(Pt(3, 5));
    const GoPoint lib1 = Pt(3, 6);
    const GoPoint lib2 = Pt(5, 5);
    BOOST_CHECK_EQUAL(OtherLiberty(bd, anchor, lib1), lib2);
    BOOST_CHECK_EQUAL(OtherLiberty(bd, anchor, lib2), lib1);
  }
  {
    const GoPoint anchor = bd.Anchor(Pt(5, 6));
    const GoPoint lib1 = Pt(5, 5);
    const GoPoint lib2 = Pt(6, 6);
    BOOST_CHECK_EQUAL(OtherLiberty(bd, anchor, lib1), lib2);
    BOOST_CHECK_EQUAL(OtherLiberty(bd, anchor, lib2), lib1);
  }
}
...
BOOST_AUTO_TEST_CASE(GoBoardUtilTest_ReduceToAnchors) {
  ...
  GoSetup setup = GoSetupUtil::CreateSetupFromString(s, boardSize);
  GoBoard bd(boardSize, setup);
  GoPointSet black = bd.All(SG_BLACK);
  BOOST_CHECK_EQUAL(black.Size(), 3);
  GoPointSet white = bd.All(SG_WHITE);
...
void CheckSelfAtari(const GoBoard& bd, GoPoint p, int nuExpectedStones) {
...
BOOST_AUTO_TEST_CASE(GoBoardUtilTest_SelfAtari_2) {
  ...
  BOOST_CHECK(!GoBoardUtil::SelfAtari(bd, Pt(8, 1)));
  BOOST_CHECK(!GoBoardUtil::SelfAtari(bd, Pt(1, 9)));

  bd.SetToPlay(SG_WHITE);
  BOOST_CHECK(!GoBoardUtil::SelfAtari(bd, Pt(1, 1)));
  BOOST_CHECK(!GoBoardUtil::SelfAtari(bd, Pt(4, 5)));
  CheckSelfAtari(bd, Pt(7, 5), 2);
  CheckSelfAtari(bd, Pt(7, 7), 2);
}
...
BOOST_AUTO_TEST_CASE(GoBoardUtilTest_SelfAtari_3) {
  ...
  CheckSelfAtari(bd, Pt(2, 9), 5);
  CheckSelfAtari(bd, Pt(8, 9), 5);
  CheckSelfAtari(bd, Pt(9, 9), 5);
  bd.SetToPlay(SG_WHITE);
  BOOST_CHECK(!GoBoardUtil::SelfAtari(bd, Pt(1, 4)));
  BOOST_CHECK(!GoBoardUtil::SelfAtari(bd, Pt(9, 2)));
...
BOOST_AUTO_TEST_CASE(GoBoardUtilTest_SelfAtari_4) {
  ...
  setup.m_player = SG_BLACK;
  GoBoard bd(boardSize, setup);
  BOOST_CHECK(!GoBoardUtil::SelfAtari(bd, Pt(4, 4)));
  CheckSelfAtari(bd, Pt(4, 2), 3);
  bd.SetToPlay(SG_WHITE);
  CheckSelfAtari(bd, Pt(3, 6), 2);
}
...
BOOST_AUTO_TEST_CASE(GoBoardUtilTest_TrompTaylorPassWins) {
  ...
  BOOST_CHECK(!TrompTaylorPassWins(bd, SG_BLACK));
  BOOST_CHECK(!TrompTaylorPassWins(bd, SG_WHITE));
  bd.Undo();
  BOOST_CHECK(TrompTaylorPassWins(bd, SG_BLACK));

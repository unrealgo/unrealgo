...
void GoTimeControl::GetPositionInfo(SgBlackWhite& toPlay,
  ...
  int finalNumEmpty = int(float(m_bd.AllPoints().Size()) * (1.f - m_finalSpace));
  estimatedRemainingMoves = max(m_bd.TotalNumEmpty() - finalNumEmpty, 3 * m_bd.Size());
  estimatedRemainingMoves /= 2;
}

void GoTimeControl::SetFinalSpace(float finalspace) {
  DBG_ASSERT(finalspace > -0.1);
  DBG_ASSERT(finalspace < 1.1);

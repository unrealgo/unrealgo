...
GoChain::GoChain(const GoChain* c1, const GoChain* c2,
  ...
  DBG_ASSERT(!cond->Overlaps(m_chainConditions));
  m_chainConditions.PushBack(cond);

  if (cond->UsesLibs()) {
    DBG_ASSERT(m_freeLiberties.Contains(cond->Lib1()));
    m_freeLiberties.Exclude(cond->Lib1());
    DBG_ASSERT(m_freeLiberties.Contains(cond->Lib2()));
...
std::ostream& operator<<(std::ostream& stream, const GoChainCondition& c) {
  ...
  if (c.UsesLibs())
    stream << ", lib1 = " << GoWritePoint(c.Lib1())
           << ", lib2 = " << GoWritePoint(c.Lib2());
  return stream;
}

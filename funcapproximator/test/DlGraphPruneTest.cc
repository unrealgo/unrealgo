...
void Test() {
  ...
  Node* inputNode = const_cast<Node*>(DlGraphUtil::GetNode(g, "input"));
  DlGraphUtil::DFS(g,
                   inputNode,
                   [&](Node* node) { preOrder.push_back(node); },
                   [&order](Node* n) { order.push_back(n); });
  CHECK_EQ(order.size(), preOrder.size());

  DlGraphUtil::PruneNodesUnreachableFrom(g, inputNode);
  CHECK_EQ(g.num_nodes(), preOrder.size());

  std::cout << "g.Nodes:" << g.num_nodes() << std::endl;
  std::cout << "graphDef.Nodes:" << graphDef.node_size() << std::endl;
  std::cout << order.size() << " nodes visited" << std::endl;

  DlGraphUtil::PruneNodesUnreachableFrom(graphDef, "input");
  std::cout << "graphDef.Nodes(after prune):" << graphDef.node_size() << std::endl;
  CHECK_EQ(g.num_nodes(), graphDef.node_size());
}

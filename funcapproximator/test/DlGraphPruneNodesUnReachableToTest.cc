...
void Test() {
  ...
  DlGraphUtil::WriteToFile(graphDef, "orig.graph.txt");
  ...
  std::cout << "size: " << graphDef.node_size() << std::endl;
  DlGraphUtil::OptimizeGraph(graphDef, names, "input");
  std::cout << "pruned size: " << graphDef.node_size() << std::endl;

  DlGraphUtil::WriteToFile(graphDef, "pruned_unreachable_to.graph.txt");
}

...
class SgSortedArray {
 public:
  ...
  void ReduceSizeTo(int newSize) {
    DBG_ASSERT(0 <= newSize && newSize <= m_numElt);
    m_numElt = newSize;
  }
 ...
 private:
  ...
  SgSortedArray(const SgSortedArray &) = delete;
  SgSortedArray &operator=(const SgSortedArray &) = delete;
};

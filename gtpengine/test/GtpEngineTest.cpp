  ...
  BOOST_AUTO_TEST_CASE(GtpCommandTest_Arg) {
    {
      ...
      BOOST_CHECK_THROW(cmd.ArgT<double>(0), GtpFailure);
      BOOST_CHECK_CLOSE(cmd.ArgT<double>(1), 9.1, 1e-4);
    }
    {
      GtpCommand cmd("command 9 abc");
      BOOST_CHECK_EQUAL(cmd.ArgT<int>(0), 9);
      BOOST_CHECK_THROW(cmd.ArgT<int>(1), GtpFailure);
    }
    {
      GtpCommand cmd("command 9 abc -1");
      BOOST_CHECK_EQUAL(cmd.ArgT<size_t>(0), 9u);
      BOOST_CHECK_THROW(cmd.ArgT<size_t>(1), GtpFailure);
      BOOST_CHECK_THROW(cmd.ArgT<size_t>(2), GtpFailure);
    }
    {
      BOOST_CHECK(!GtpCommand("command 0").ArgT<bool>(0));
      BOOST_CHECK(GtpCommand("command 1").ArgT<bool>(0));
      BOOST_CHECK_THROW(GtpCommand("command 2").ArgT<bool>(0), GtpFailure);
      BOOST_CHECK_THROW(GtpCommand("command foo").ArgT<bool>(0), GtpFailure);
      BOOST_CHECK_THROW(GtpCommand("command").ArgT<bool>(0), GtpFailure);
    }
  }

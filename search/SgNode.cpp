...
SgVector<GoPoint> SgNode::VectorProp(SgPropID prop) const {
  SgPropPointList *lp = dynamic_cast<SgPropPointList *>(Get(prop));
  if (lp)
    return lp->Value();
  return SgVector<GoPoint>();
}
...
SgNode *SgNode::LeftBrother() const {
  if (!HasFather() || !HasLeftBrother())
    return 0;
  SgNode *node = Father()->LeftMostSon();
...
SgNode *SgNode::NodeInDirection(Direction dir) const {
  SgNode *node = 0;
  switch (dir) {
    case PREVIOUS:node = Father();
      break;
    case NEXT:node = LeftMostSon();
      break;
    case NEXT_RIGHTMOST:node = RightMostSon();
      break;
    case PREV_DEPTHFIRST:node = PrevDepthFirst();
      break;
    case NEXT_DEPTHFIRST:node = NextDepthFirst();
      break;
    case PREV_TERMINAL:node = PrevDepthFirst();
      while (!node->IsTerminal())
        node = node->PrevDepthFirst();
      break;
    case NEXT_TERMINAL:node = NextDepthFirst();
      while (!node->IsTerminal())
        node = node->NextDepthFirst();
      break;
    case PREV_BRANCH:node = Father();
      while (node && node->HasFather() && !node->IsBranchPoint())
        node = node->Father();
      break;
    case NEXT_BRANCH:node = LeftMostSon();
    ...
    case LEFT_BROTHER:
      if (HasLeftBrother())
        ...
        node = Father()->RightMostSon();
      break;
    case RIGHT_BROTHER:
      if (HasRightBrother())
        node = RightBrother();
      else if (HasBrother())
        node = Father()->LeftMostSon();
      break;
    case MAIN_BRANCH: {
      ...
      break;
    case START_OF_GAME:node = Root();
      break;
    case END_OF_GAME:node = const_cast<SgNode *>(this);
      while (!node->IsTerminal())
        node = node->LeftMostSon();
      break;
...
bool SgNode::ContainsText(const string &findText) {
  DBG_ASSERT(SgProp::ConvertFindTextToPropID(findText) == SG_PROP_NONE);
  return m_props.GetPropContainingText(findText) != 0;
}
void SgNode::AddComment(const std::string &comment) {
  SgPropText *textProp = dynamic_cast<SgPropText *>(Get(SG_PROP_COMMENT));
  if (textProp)
...
void SgNode::PromoteNode() {
  if (HasLeftBrother()) {
    ...
    m_brother = m_father->m_son;
    m_father->m_son = this;
    SgNode *x = this;
    while (x->m_brother != this)
      x = x->m_brother;
    x->m_brother = brotherOfThis;
  }
}
...
void SgNode::AppendTo(SgNode *n) {
  DBG_ASSERT(!HasFather());
  DBG_ASSERT(!HasBrother());
  DBG_ASSERT(n);
  if (n->HasSon()) {
...
SgBlackWhite SgNode::NodePlayer() const {
  DBG_ASSERT(HasNodeMove());
  if (HasProp(SG_PROP_MOVE_BLACK))
    return SG_BLACK;
  return SG_WHITE;
}

GoPoint SgNode::NodeMove() const {
  GoPoint p;
...
void SgNode::SetListProp(SgPropID id, const SgVector<GoPoint> &value) {
...
void SgNode::SetListProp(SgPropID id, const GoPointSet &value) {
  SgVector<GoPoint> valueList;
...
SgProp *SgNode::AddMoveProp(GoMove move, SgBlackWhite player) {
...
#ifndef NDEBUG
int SgNode::s_allocated = 0;
int SgNode::s_freed = 0;

void SgNode::GetStatistics(int *numAlloc, int *numUsed) {
  *numAlloc = s_allocated;
  *numUsed = s_allocated - s_freed;
}
#endif
  ...
  if (m_nextNode) {
    if (m_postOrder) {
      if (m_nextNode == m_rootOfSubtree)
        ...
        DBG_ASSERT(m_nextNode->HasFather());

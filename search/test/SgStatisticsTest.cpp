...
BOOST_AUTO_TEST_CASE(UctStatisticsBaseTest_CheckAddRemoveCount) {
  UctStatisticsBase<double, double> statistics;
  statistics.Add(2., 1.);
  ...
  UctStatisticsBase<double, std::size_t> stat;
  stat.Add(0.0, 1);
  stat.Add(1.0, 2);
  stat.Add(0.5, 4);
...
BOOST_AUTO_TEST_CASE(UctStatisticsBaseTest_CheckRemove) {
  UctStatisticsBase<double, std::size_t> stat;
  stat.Add(2.0);
  stat.Remove(2.0);
  BOOST_CHECK(!stat.IsDefined());
